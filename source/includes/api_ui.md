...
|API|参数|返回值|平台|备注|
|----|----|----|----|----|
|drawLine|x1: Number<br/> y1: Number<br/> x2: Number<br/> y2: Number<br/>|-|-|绘制线条。(x1,y1), (x2, y2)分别为起点终点|
|drawPoint|x: Number<br/> y: Number<br/>|-|-|绘制点|
|drawRect|x: Number<br/> y: Number<br/> w: Number<br/> h: Number<br/>|-|-|绘制矩形。x起点x坐标，y起点y坐标，w宽度，h高度|
|drawRoundRect|x: Number<br/> y: Number<br/> w: Number<br/> h: Number<br/> rx: Number<br/> ry: Number<br/>|-|-|绘制圆角矩形。x起点x坐标，y起点y坐标，w宽度，h高度，rx为x轴圆角半径，ry为y轴圆角半径|
...
|drawArc|x: Number<br/> y: Number<br/> w: Number<br/> h: Number<br/> startAngle: Number<br/> sweepAngle: Number<br/> useCenter: Boolean|-|-|绘制扇形。(x,y)为左上角坐标，(w,h)为扇形宽高，startAngle为开始角度，sweepAngle为覆盖角度, useCenter为是否覆盖完整扇形面积（默认为false）|
|drawText|text: String/<a href="#styled_string">StyledString<a/>/<a href="#unicode">Unicode<a/> <br/> x: Number<br/> y:Number|-|-|绘制文本|
|drawImage|image: String/<a href="#image">Image</a> <br/> x: Number<br/> y: Number<br/> w: Number<br/> h: Number|-|-|绘制图片，支持本地图和Image对象|
...
|style|style: <a href="#paint_style">PaintStyle</a>|-|-|设置画笔填充样式|
...
|skew|x: Number<br/> y: Number<br/>|-|-|斜切变换，xy斜切比率|
